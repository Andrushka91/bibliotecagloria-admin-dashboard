...
export default function BookForm({ book, bookId, itemEdited, setItemEdited, handleModalEditClose }) {
    const classes = useStyles();

    const { state, updateBook } = useContext(BooksContext);
    ...
    useEffect(() => {
        ...
        const isEmpty = Object.values(book).every(x => x === null || x === '');
    ...
    const submitBook = () => {
        ...
        if (isEmpty) {
            ...
            setValidate(false);
            updateBook(handleToggle, form, itemEdited, setItemEdited, bookId, handleModalEditClose, handleClose);
            // setTimeout(() => {
            //     setSuccess(false);
